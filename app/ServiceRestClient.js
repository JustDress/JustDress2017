angular.module("app.proxy", []).factory("RestClient", ["$http", "config", "$localStorage", "$filter", "$rootScope",
    function($http, config, $localStorage, $filter, $rootScope){
        ...
        function makeRequest(req, callBack){
          ...
          /*  if(($localStorage.accessToken !== null && $localStorage.accessToken != undefined) && (req.data === undefined || (req.data !== undefined && / req.data.grant_type === undefined))){
                req.headers = {
        ...
        return {
            products : function(callBack){
                getRequest("/products/all", callBack);
            }
        };
    }
]);
